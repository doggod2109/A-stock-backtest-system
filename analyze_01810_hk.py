...
def get_hk_realtime_data(stock_code='01810'):
    ...
    try:
        ...
        # 腾讯数据字段映射 (验证格式: 字段0=状态, 字段1=名称, 字段2=代码, 字段3=当前价, 字段4=昨收, 字段5=开盘, 字段6=成交量)
        # 字段34=时间, 字段35=涨跌额, 字段36=涨跌幅, 字段37=最高, 字段38=最低, 字段39=成交额
        real_data = {
            'name': parts[1],
            'code': parts[2],
            ...
            'high': float(parts[37]) if len(parts) > 37 and parts[37] else 0,
            'low': float(parts[38]) if len(parts) > 38 and parts[38] else 0,
            'turnover': float(parts[39]) if len(parts) > 39 and parts[39] else 0,
            'timestamp': parts[34] if len(parts) > 34 else '',
            'change': float(parts[35]) if len(parts) > 35 and parts[35] else 0,
            'pct_chg': float(parts[36]) if len(parts) > 36 and parts[36] else 0,
            'pe': 0,  # 腾讯数据不直接提供PE
        }
...
def get_hk_historical_data(stock_code='01810', period='1y'):
    """获取港股历史K线数据 - 使用新浪财经港股API"""
    try:
        # 新浪财经港股API
        ...
        # 标准化列名 (新浪返回列名可能是中文)
        # 检查实际列名
        print(f"新浪财经返回的列名: {df.columns.tolist()}")

        # 尝试标准化列名
        column_map = {}
        for col in df.columns:
            col_lower = str(col).lower()
            if 'day' in col_lower or '日期' in col:
                column_map[col] = 'date'
...
def main():
    ...
    # 2. 获取历史数据
    print("📊 获取历史K线数据...")
    df = get_hk_historical_data('01810', '1y')
